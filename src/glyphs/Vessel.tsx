...
/**
 * The sheen's left edge `p` of the way through a sweep across a pill
 * `width` wide: in from beyond one end and out past the other, right to
 * left when `reversed`.
 */
export function sheenX(p: number, width: number, reversed: boolean): number {
  'worklet';
  const travel = width + SHEEN_WIDTH;
  return reversed ? width - p * travel : p * travel - SHEEN_WIDTH;
}
...
export function Vessel({
  ...
  const sheenStyle = useAnimatedStyle(
    () => ({
      transform: [{ translateX: sheenX(fract(sheen.get()), width, reversed) }],
    }),
    [width, reversed],
  );
  const rippleStyle = useAnimatedStyle(() => {
    ...
    return {
      ...
      transform: [
        { translateX: share.get() * width - SHEEN_WIDTH / 4 },
        { scaleX: pose.scale },
      ],
    };
  }, [width]);
    ...
    >
      <LayoutAnimationConfig skipEntering>
        <Reanimated.View
          style={[
            styles.pill,
            { backgroundColor: split ? FILLS[visual.fill] : HAIRLINE },
            pillStyle,
            dimStyle,
          ]}
        >
          {split && reduced && visual.fill !== 'seeds' ? <Hatch /> : null}
        ...
        {glyph ? (
          ...
          >
            ...
            >
              <Glyph name={glyph} size={16} color={tone} />
            </View>
          </Reanimated.View>
        ) : null}
      </LayoutAnimationConfig>
...
const styles = StyleSheet.create({
  ...
  // between the parts stays straight.
  solid: {
