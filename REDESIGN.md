...
| `glyphs/Vessel`           | `availableSats`, `pendingSats`, `lfbw?`, `unit`, `masked?`, `stale?`                                                                                                                                                                                                                                                       | Reads "{a} sats ready to send, {p} sats arriving". The look comes from the pure `vesselVisual(balance, lfbw)` in `scenes/home/visual.ts`: `{ weight, solid, fill, sheen, glyph, tone, retry }`. Its value names the wait (`vesselWords(visual)`), which its glyph whispers.                                                                                                                                                                                                                                                                                  |
