...
- **Request frames.** The same ring runs around the QR frame's perimeter.
...
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `glyphs/Bloom`            | `size`, `mode?` `'still' \| 'breathe' \| 'chase' \| 'ratchet'`, `open?` (0 to 1), `tone?` `'live' \| 'test' \| 'dormant'`, `halo?`, `event?` `{ kind: 'burst' \| 'wilt' \| 'fold' \| 'fall' \| 'shake'; key }`, `detail?` `'full' \| 'mark'`, `opening?` (a shared value, 0 to 1), `accessibilityLabel?` | Decoration unless labelled, then one `image`. `detail` defaults to `mark` below 40pt. An `event` plays when its `key` changes. `opening` is a pull read on the UI thread: past 0 the petals fold and one more opens on the reveal spring each twelfth of the way (`pulledPetal`), and at 0 they return to `open`. |
| `glyphs/Odometer`         | `sats`, `unit`, `masked?`, `stale?`, `variant` `'hero' \| 'amount' \| 'amountDetail' \| 'line' \| 'row'`, `color?`, `sign?` `'+' \| '-' \| null`, `accessibilityLabel?`                                                                                                                                  | One accessible element; reads the amount in sats by default, "Amount hidden" when masked. Also exports the pure worklet `digitPosition(v, k)` and `cellsFor(sats, unit)` (cells keyed `d{place}` by place value, BTC always 8 decimals with trailing zeros `dim`).                                                |
...
| `glyphs/StatusRing`       | `size` `40 \| 96 \| 120`, `visual: RingVisual`                                                                                                                                                                                                                                                           | Decoration. `RingVisual` is `{ tone, pattern, progress?, split?, glyph, badge? }`, from the pure `ringVisual(activity)` in `scenes/activity/visual.ts`. Uncertain is honey `held` with `pause`, and never shares a tone or pattern with a completed ring.                                                         |
| `glyphs/CopyChip`         | `label`, `value`, `glyph?`                                                                                                                                                                                                                                                                               | Label "Copy {label}" (lowercased, as `CopyValue`). A tap copies and announces "{Label} copied"; a long press shows the whole value.                                                                                                                                                                               |
| `glyphs/HoldButton`       | `accessibilityLabel`, `onCommit`, `warning?`, `disabled?`, `busy?`, `children?`                                                                                                                                                                                                                          | No `onPress`: a tap never commits. The hold is 700ms, 1000ms with `warning`. `accessibilityActions` is `[{ name: 'activate' }]`, so a test commits with `onAccessibilityAction({ nativeEvent: { actionName: 'activate' } })`.                                                                                     |
| `glyphs/ExpiryRing`       | `size`, `expiresAt`, `createdAt?`, `shape?` `'circle' \| 'rect'`, `width?`, `height?`, `radius?`, `onExpired?`                                                                                                                                                                                           | Decoration. Honey at 10s or less; `onExpired` fires once.                                                                                                                                                                                                                                                         |
