...
- **Native files of the privacy cover** (6, the native cover and Android recents): on iOS `ios/chicory/PrivacyCover.{h,m}`, which `src/stage/nativeCover.ts` tells, with the bridging header that shows it to Swift (`ios/chicory/chicory-Bridging-Header.h`), and the cover in `ios/chicory/AppDelegate.swift`; on Android the recents rule in `android/app/src/main/java/com/chicory/MainActivity.kt`. They belong to the privacy cover and are shared, as the stage is; `__tests__/NativePrivacyCover.test.ts` reads them.
