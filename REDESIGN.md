...
- **Announcements.** A single `announce(text, {assertive})` helper.
  - The safety states announce assertively, once: an element whose words are announced this way is not also a live region, which Android would read a second time.
