...
**The native cover.** iOS starts the app switcher's animation from a picture it takes as the app goes inactive, before React can draw, so the stage's cover came too late for the outgoing card and the balance showed in it for about 180ms. So the app delegate puts a plain roast view over the window in `applicationWillResignActive` itself, above the root view and at once, unless a prompt the app raised may be up or the lock is what is drawn. In `applicationDidEnterBackground` it puts it up whatever it was told, since no prompt outlasts going to the background, and `applicationDidBecomeActive` takes it away. It cannot ask JavaScript at that moment, so it is told, through the `PrivacyCover` module's flags, each set by a blocking synchronous call (`src/stage/nativeCover.ts`): `duringSystemPrompt` sets the prompt flag as its span opens, before the call that raises the prompt, and clears it once the last span has settled; the stage sets the lock flag while the lock is drawn, so the bud stays in view behind the lock's own Face ID prompt. On iOS the switcher's picture is that plain roast; the stage's cover with its mark is drawn beneath it, and covers alone wherever there is no native one.
...
- **Native files of the privacy cover** (6, the native cover): `ios/chicory/PrivacyCover.{h,m}`, which `src/stage/nativeCover.ts` tells, with the bridging header that shows it to Swift (`ios/chicory/chicory-Bridging-Header.h`), and the cover in `ios/chicory/AppDelegate.swift`. They belong to the privacy cover and are shared, as the stage is; `__tests__/NativePrivacyCover.test.ts` reads them.
