...
6. **Held requests.** A request whose earlier payment is still pending or uncertain cannot be paid again. It lands on the held ring instead of a review. It is held from the moment its hold commits, before the payment's call has answered, and nothing the history shows lets it go until that call does (`src/stage/heldRequests.ts`).
