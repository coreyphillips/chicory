...
| **Submarine swap** (`client.swaps.submarine`) | a node with on-chain coins that wants Lightning balance | a provider with `BEIGNET_SWAP_SUBMARINE` on | Your node mints an invoice. Chicory verifies the contract the provider offers (built from your refund key and the provider's claim key) and funds it from your wallet. The provider pays your invoice under an absolute HTLC expiry ceiling and claims the coins with the preimage. Unpaid past the refund height, chicory refunds you, never while your node still holds the provider's HTLC. |

All four use beignet's custom peer protocols. Chicory reuses beignet's codecs and messages; the provider must support the requested protocol. This is not a Boltz API client.
...
### Submarine swaps: what chicory verifies, funds and refunds
...
`createLndClient({ host, port, macaroonHex, network, storage, chain? })` and `createClnClient({ host, port, rune, network, storage, chain? })` build a client whose link, wallet, Lightning payer and funder are one node; `import ... from 'chicory/lnd'` or `'chicory/cln'` gives the adapters without the rest, and `'chicory/ldk'` the bridge link.
...
Swap seams: LndPayer({ host, port, macaroonHex, network })     ClnPayer({ host, port, rune, network })
            ...
            swaps: { payer, funder?, chain, policy?, destination? }; policy defaults in SWAP_DEFAULT_POLICY
